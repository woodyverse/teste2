...
export default class HelloWorld {
	...
	private async started() {
		...
		const setaData = await this.assets.loadGltf('Seta.glb', "box");
		const cubeData = await this.assets.loadGltf('Botao_Azul.glb', "box");
		...
		buttonBehavior.onClick(_ => {
			this.texto.text.contents = _.name;
			
			this.seta = MRE.Actor.CreateFromPrefab(this.context, {
				// using the data we loaded earlier
				firstPrefabFrom: setaData,
				// Also apply the following generic actor properties.
				actor: {
					name: 'Seta Escolhido',
					// Parent the glTF model to the text actor, so the transform is relative to the text
